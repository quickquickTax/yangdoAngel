...
1. 직접 식별정보 없이 필요한 거래 정보를 질문으로 수집합니다.
...
- 상속·증여 평가기간 산정과 실거래가·공시가격 후보 조회
- 상속재산의 장기공제·세율 보유기간 분리 계산
- 배우자·직계존비속 증여의 일반 이월과세 계산
- 계산 전 누락값 질문과 위험 체크리스트 생성
- 1세대 1주택 비과세 요청의 일관성 검증
- 규칙 적용기간과 양도일 불일치 차단
- 부담부증여와 동일 연도 복수 양도 등 미지원 사건 차단
- 증빙이 확인되지 않은 필요경비 차단
...
상속·증여 평가기간을 계산하고 공식 실거래가·공시가격 API에서 취득가액 후보를 조회합니다. 선택 금액, 근거, 신뢰도, 공식 확인 링크와 `caseDataPatch`를 반환합니다. 공동주택 공시가격은 동·호와 취득연도가 일치할 때만 자동 선택하며, 유사매매사례는 전용면적과 공동주택가격 차이가 각각 5% 이내로 확인되어야 합니다. 2023년 이전 취득은 신고·결정·경정된 확정 평가액이 있어야 합니다.
...
- 상속·증여 평가 근거, 피상속인 취득일, 증여 이월과세 정보
- 부담부증여, 동일 연도 복수 양도 등 미지원 위험
- 필요경비와 증빙 보유 여부
...
- 필수 annotations 제공
  ...
  - `openWorldHint=false` (평가가액 외부 조회 도구는 `true`)
  - `idempotentHint=true`
...
- 상속·증여 취득 주택의 1세대1주택 비과세 자동 판정
- 배우자·직계존비속 외 특수관계인 증여의 부당행위계산
- 조합원입주권과 분양권
- 감면, 가산세, 외국납부세액
- 국외 자산, 법인과 비거주자
- 전자신고와 신고서 자동 제출
