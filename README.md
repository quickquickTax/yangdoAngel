...
AI는 금액 답변이 들어오면 `normalize_amount_input`으로 원 단위 숫자로 변환한 뒤 사용자 답변을 `caseData`에 누적합니다. 이후 `prepare_capital_gains_case_checklist`, `validate_capital_gains_case` 순서로 호출하고, 검증을 통과한 경우에만 `calculate_capital_gains_tax`를 호출해야 합니다.
