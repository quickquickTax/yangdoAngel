...
사용자 답변을 누적한 `caseData`를 기준으로 계산 전 체크리스트와 질문 그룹을 생성합니다.
...
반환값에는 기존 `questions` 배열과 함께 `questionGroups`가 포함됩니다. `questionGroups`는 거래 정보, 자산 정보, 소유 형태, 세대 및 비과세, 과세연도 맥락, 필요경비 및 증빙, 지원 범위, 검증 결과 확인 순서로 질문을 묶어 반환합니다.
